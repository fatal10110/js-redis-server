...
- `DISCARD` cancels a transaction; `EXECABORT` is returned if the queue itself is dirty. Only what Redis's `processCommand` refuses dirties it: an unknown command or subcommand, or an argument count the command table rejects, from 7.0 the real `container|subcommand` entry's (`CommandExecutor.planForQueue`, `containerSubcommandArity`). Any other error the parser raises is the command's own argument check, so the command is queued as a plan carrying `deferredError` (raised after the policy chain at EXEC) and routed by the keys Redis's `getKeysFromCommand` finds in the raw arguments — the command's getkeys procedure (`CommandDefinition.rawKeys`) else the legacy key range (`src/core/key-specs.ts`)
