...
Today a developer points `ioredis` at the js-redis-server mock, loads a Lua script, runs `EVAL`, and has **no way to step through the Lua** — only `print`/log. We own the Lua interpreter as a WASM package (`redis-lua-wasm`, Lua 5.1 via Emscripten; source in the sibling internal repo `../redis-lua-wasm`) *and* the mock that drives it, so we can wire a real VS Code debugging experience end-to-end: gutter breakpoints, step in/over/out, call stack, a Variables panel (KEYS/ARGV/locals/upvalues/nested tables), and watch/evaluate — all while the script executes inside the normal mock `EVAL` pipeline (`redis.call` still hits the live keyspace).
 ...
 step, vars)                 │ drives
        ...
        ioredis ─EVAL─► RedisLuaRuntime ─► debug WASM ─┘
                             ...
                             └─ redis.call ◄────┤  user chunk  (chunkname = SHA)
                                (sync, live      │  + Lua "debug agent" (debug/fuel hook + loop)
                                 keyspace)
```

**Pause mechanism (the key idea).** The debug build uses **one combined hook**, installed from C with `LUA_MASKCOUNT | LUA_MASKLINE`. Lua has only one hook slot per state, so the debug hook must preserve the existing fuel behavior on count events and delegate line events to the Lua debug agent. When the line side decides to stop (breakpoint hit / step landed), the agent enters a **command loop** that round-trips to JS via one Asyncify host import, `host_debug_request`. Because suspension always happens *inside the agent's host call* — never unwinding the user's frames — the user's Lua stack and locals stay **live**. That gives full-fidelity inspection: lazy table expansion and expression `evaluate` in the paused frame both work, with no Worker/SharedArrayBuffer. The C surface stays small; the debugger "brain" lives in the Lua agent + the TypeScript DAP layer.
...
1. **New build flavor** — `wasm/build/build.sh` (or a sibling `build-debug.sh`): emit `redis_lua.debug.{mjs,wasm}` with existing flags **plus**:
   ...
   - update `scripts/copy-wasm.mjs`, package `files`, and path-resolution tests so both `redis_lua.{mjs,wasm}` and `redis_lua.debug.{mjs,wasm}` are copied to `dist/` and published.
...
2. **`wasm/src/runtime.c`**
   ...
   - Add host import decl in `wasm/include/abi.h`: `extern PtrLen host_debug_request(uint32_t ptr, uint32_t len);` (the one Asyncify import).
   ...
   - `_eval_debug(...)`: same as `eval_with_args`, but first `luaL_loadbuffer`+`lua_pcall` the **agent chunk** (registers agent state/functions), then install a combined C hook with `lua_sethook(g_state, debug_fuel_hook, LUA_MASKCOUNT | LUA_MASKLINE, FUEL_HOOK_STEP)`, then run the user chunk.
   - `debug_fuel_hook` handles count events by decrementing the same fuel counter used today and raising the same fuel-limit error. It handles line events by calling the agent's line callback with the current line/stack depth. The agent must **not** call `debug.sethook`; that would replace the fuel hook.
   - On every `_eval_debug` exit path (success, Lua error, host error, abort/cancel), clear debug globals/handle tables and restore the normal fuel-only hook so a later non-debug eval is not left in debug mode.
...
3. **Lua debug agent** — `wasm/src/debug_agent.lua` embedded as a C string (or a `_load_debug_agent` export). Pure Lua, owns all introspection:
   - Exposes a line callback called by the combined C hook; it checks local `breakpoints[line]` / step state and enters the stop loop only when needed.
   - On stop: ping-pong loop calling `__redis_debug_request(json)`; handles ops `stackTrace` (`debug.getinfo`), `scopes`/`variables` (`debug.getlocal`, `debug.getupvalue`, recursive table walk with `variablesReference` handles), and expression `evaluate`.
   - `evaluate` builds a paused-frame environment explicitly: collect locals/upvalues for the target frame, fall back to `_G` via `__index`, compile with `loadstring("return " .. expr)` (fallback to `loadstring(expr)` only for future statement support), then `setfenv(fn, env)` and `pcall(fn)`. V1 expression evaluation is read-oriented; assignment/write-back needs explicit `debug.setlocal`/upvalue rules before being advertised.
   - The stop loop exits on a `resume{mode, breakpoints}` reply, updating local state; `mode` ∈ continue/stepOver/stepIn/stepOut via `debug.getinfo` stack-depth tracking.
   - One `ready` handshake at entry (supports `stopOnEntry`).
   - ABI: reuse the existing length-prefixed buffer convention (`helpers.ts`/`codec.ts`); payloads are JSON (perf is a non-goal per `AGENTS.md`).
...
4. **JS API** — `src/engine.ts` + `src/loader.ts` + `src/types.ts`:
   ...
   - Extend `HostImport`/ABI helpers for the Asyncify import: `host_debug_request` may return a `Promise` and must still support the current direct/sret `PtrLen` conventions.
   - `module.create(host)` accepts optional `onDebugRequest(bytes, signal?): Promise<bytes>` (async — allowed because it's in `ASYNCIFY_IMPORTS`). If omitted, debug eval fails with a clear error.
...
1. **`src/core/lua-runtime.ts`** — `RedisLuaRuntime` gains a debug mode:
   - Add `RedisLuaRuntimeOptions` / `createRedisLuaRuntime(options?)`. Construct with the **debug** WASM (`load({ wasmPath, modulePath: debug paths })`) when debugging is enabled; otherwise unchanged.
   - Provide `onDebugRequest` that forwards to a `LuaDebugController` (Part C) and awaits its decision. Thread `ctx.signal` into the controller so disconnect/server shutdown can cancel a paused script.
   ...
   - Keep `hostState.ctx` set until the awaited debug eval settles, then clear it in `finally`; otherwise a paused script can leak re-entrancy state after an abort.
...
- **`lua-debug-controller.ts`** — bridge between `onDebugRequest` (Part B) and the adapter:
  ...
  - `disconnect`/abort → resolve the pending `host_debug_request` with `cancel{reason}` or reject it with a debugger-cancelled error; do not leave the Lua runtime waiting forever.
  - Owns `variablesReference` handle table mirroring the agent's.
- **Protocol** (over `host_debug_request`): agent→host `ready|stopped|result|error`; host→agent `inspect{op,args}|resume{mode,breakpoints}|cancel{reason}`. JSON with request ids so one in-flight inspect/evaluate can be correlated and failures are explicit.

---

## Part D — VS Code wiring (adapter-only)

- **`src/debug/server.ts`** — `createLuaDebugServer(opts)` starts the adapter in **DAP server mode** (`net` socket) bound to the state's `LuaDebugController` / `DebugSourceRegistry`; logs `lua-debug adapter listening on <port>`. Exported from `src/index.ts`. A `bin` entry allows spawning standalone too.
- **Developer flow**: run the existing ioredis test/app with `REDIS_LUA_DEBUG=1` (or construct `new RedisServerState({ luaDebug: ... })`) → adapter server comes up. In VS Code, `.vscode/launch.json`:
  ```json
...
1. **WASM**: build the debug flavor; a node smoke test runs `engine.evalDebug("local x=1\nreturn x", ...)` with an `onDebugRequest` stub that (a) sets a breakpoint on line 2, (b) on `stopped` issues `variables`, asserts `x==1`, (c) `evaluate("x+41")`==42 via the explicit frame env, (d) resumes → result 1. Add a second smoke test for `while true do end` under debug to prove the combined hook still enforces fuel.
2. **Unit** (`tests/`, node:test): `LuaDebugController` protocol translation — fake agent ping-pong ⇄ DAP requests; breakpoint set/clear; step modes; variablesReference expansion of a nested table; disconnect/abort cancels a pending pause.
3. **Integration** (`tests-integration/`): start a mock server with `RedisServerStateOptions.luaDebug`, connect a scripted DAP client (`@vscode/debugadapter-testsupport`) to the server socket, `ioredis.eval(script,...)` on another connection; assert: stop on breakpoint, `redis.call` inside the script still mutates the live keyspace, stackTrace/scopes/variables payloads, evaluate, continue → correct EVAL reply. Add a disconnect test that closes DAP while paused and verifies the EVAL unwinds and a later normal command runs. **Mock backend only** (no WASM on real-Redis backend); keep the rest of `test:all` green.
...
- **Hook ownership**: Lua has one hook slot per state. The debug hook must be the sole installed hook during `_eval_debug`, and it must preserve fuel semantics itself.
- **State reuse**: `g_state` is a singleton reused across evals; re-install the combined hook + agent globals per debug eval and tear down after, so a later non-debug eval isn't left with a line hook or stale handles.
...
- **Cancellation**: a paused script holds the DB turn by design. Any DAP/session/server close path must unblock `host_debug_request`; otherwise the keyspace can remain permanently blocked.
- **Packaging**: the debug `.mjs`/`.wasm` pair must be copied to `dist/` and covered by default debug path tests before publishing `lua-redis-wasm`.
- **Performance**: per-line hook + JSON ping-pong is slow, but debug-only and explicitly acceptable (`AGENTS.md`: perf not a priority).
