...
describe('socketless client matches the wire client on a tcp mock (node-redis)', () => {
  let mock: RedisMock
  let wire: RedisClientType

  before(async () => {
    mock = await createRedisMock()
    await mock.seed(SEED)
    wire = createClient({ url: mock.url }) as RedisClientType
    await wire.connect()
  })
...
describe('memory-transport mock seed → socketless read-back (node-redis)', () => {
  let mock: RedisMock
