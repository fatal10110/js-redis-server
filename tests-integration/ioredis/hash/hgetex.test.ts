...
describe(`Hash Commands Integration (${testRunner.getBackendName()})`, () => {
  ...
  test('HGETEX returns values and updates field TTLs like Redis', async () => {
    ...
    try {
      ...
      assert.deepStrictEqual(
        await directClient.hgetex(key, 'FIELDS', '3', 'f1', 'missing', 'f2'),
        ['v1', null, 'v2'],
      )
      // Missing key yields all-nil without creating the hash.
      assert.deepStrictEqual(
        await directClient.hgetex(`${tag}:nokey`, 'FIELDS', '2', 'a', 'b'),
        [null, null],
      )
      ...
      assert.deepStrictEqual(
        await directClient.hgetex(key, 'EX', '100', 'FIELDS', '1', 'f1'),
        ['v1'],
      )
      ...
      assert.deepStrictEqual(
        await directClient.hgetex(key, 'FIELDS', '1', 'f1'),
        ['v1'],
      )
      const ttlKeep = (await directClient.httl(
        key,
        'FIELDS',
        '1',
        'f1',
      ...
      assert.deepStrictEqual(
        await directClient.hgetex(key, 'PERSIST', 'FIELDS', '1', 'f1'),
        ['v1'],
      )
      assert.deepStrictEqual(
      ...
      await directClient.hgetex(key, 'PX', '50000', 'FIELDS', '1', 'f2')
      ...
      assert.deepStrictEqual(
        await directClient.hgetex(key, 'EXAT', '1', 'FIELDS', '1', 'f3'),
        ['v3'],
      )
      ...
      assert.deepStrictEqual(
        await directClient.hgetex(key, 'EX', '0', 'FIELDS', '1', 'f4'),
        ['v4'],
      )
      ...
      assert.deepStrictEqual(
        await directClient.hgetex(lone, 'EXAT', '1', 'FIELDS', '1', 'only'),
        ['v'],
      )
  ...
  test('HGETEX errors match Redis', async () => {
    ...
    try {
      ...
      await assert.rejects(
        () => directClient?.hgetex(hashKey, 'FIELD', '1', 'field'),
        errorWithMessage(
      ...
      await assert.rejects(
        () => directClient?.hgetex(hashKey, 'KEEPTTL', 'FIELDS', '1', 'field'),
        errorWithMessage(
          'ERR Mandatory argument FIELDS is missing or not at the right position',
        ),
      )
      ...
      await assert.rejects(
        () => directClient?.hgetex(hashKey, 'FIELDS', 'abc', 'field'),
        errorWithMessage('ERR Number of fields must be a positive integer'),
      )
      await assert.rejects(
        () => directClient?.hgetex(hashKey, 'FIELDS', '0', 'field'),
        errorWithMessage('ERR Number of fields must be a positive integer'),
      )
      await assert.rejects(
        () => directClient?.hgetex(hashKey, 'FIELDS', '2', 'field'),
        errorWithMessage(
      ...
      await assert.rejects(
        () =>
          directClient?.hgetex(hashKey, 'EX', 'abc', 'FIELDS', '1', 'field'),
        errorWithMessage('ERR value is not an integer or out of range'),
      )
      await assert.rejects(
        () => directClient?.hgetex(hashKey, 'EX', '-5', 'FIELDS', '1', 'field'),
