...
describe(`LPOS / LMOVE Integration (${testRunner.getBackendName()})`, () => {
  ...
  test('LPOS finds matches with RANK, COUNT and MAXLEN', async () => {
    ...
    try {
      ...
      assert.strictEqual(await client.call('LPOS', key, 'a'), 0)

      // RANK 2 -> second match
      assert.strictEqual(await client.call('LPOS', key, 'a', 'RANK', '2'), 3)

      // negative RANK -> first match scanning tail -> head (absolute index)
      assert.strictEqual(await client.call('LPOS', key, 'a', 'RANK', '-1'), 6)

      // COUNT 0 -> all matches as array, ascending
      assert.deepStrictEqual(
        await client.call('LPOS', key, 'a', 'COUNT', '0'),
        [0, 3, 6],
      )

      // COUNT N -> first N matches
      assert.deepStrictEqual(
        await client.call('LPOS', key, 'a', 'COUNT', '2'),
        [0, 3],
      )

      // negative RANK + COUNT -> scan from tail, descending indexes
      assert.deepStrictEqual(
        await client.call('LPOS', key, 'a', 'RANK', '-1', 'COUNT', '2'),
        [6, 3],
      )

      // MAXLEN limits elements scanned from head; only index 0,1 examined
      assert.deepStrictEqual(
        await client.call('LPOS', key, 'a', 'MAXLEN', '2', 'COUNT', '0'),
        [0],
      )
      ...
      assert.strictEqual(await client.call('LPOS', key, 'z'), null)

      // missing element, COUNT 0 -> empty array
      assert.deepStrictEqual(
        await client.call('LPOS', key, 'z', 'COUNT', '0'),
        [],
      )

      // nonexistent key, no COUNT -> nil
      assert.strictEqual(await client.call('LPOS', `${tag}:missing`, 'a'), null)

      // nonexistent key, COUNT 0 -> empty array
      assert.deepStrictEqual(
        await client.call('LPOS', `${tag}:missing`, 'a', 'COUNT', '0'),
        [],
      )
    } finally {
      await client.del(key)
  ...
  test('LPOS error and edge paths match Redis', async () => {
    ...
    try {
      ...
      await assert.rejects(
        () => client.call('LPOS', key, 'a', 'COUNT', '-1'),
  ...
  test('LMOVE moves elements between list ends', async () => {
    ...
    try {
      ...
      assert.strictEqual(
        await client.call('LMOVE', src, dst, 'LEFT', 'RIGHT'),
        'a',
      )
      ...
      assert.strictEqual(
        await client.call('LMOVE', src, dst, 'RIGHT', 'LEFT'),
        'c',
      )
  ...
  test('LMOVE rotates a list onto itself', async () => {
    ...
    try {
      ...
      assert.strictEqual(
        await client.call('LMOVE', key, key, 'RIGHT', 'LEFT'),
        '3',
      )
  ...
  test('LMOVE error paths match Redis', async () => {
    ...
    try {
      ...
      await client.set(stringKey, 'value')
      await assert.rejects(
        () => client.call('LMOVE', stringKey, dst, 'LEFT', 'RIGHT'),
        errorWithMessage(
          'WRONGTYPE Operation against a key holding the wrong kind of value',
        ),
      )
      ...
      await assert.rejects(
        () => client.call('LMOVE', src, stringKey, 'LEFT', 'RIGHT'),
        errorWithMessage(
          'WRONGTYPE Operation against a key holding the wrong kind of value',
        ),
      )
