...
describe(`Sorted Set Set-Operations (${testRunner.getBackendName()})`, () => {
  ...
  test('ZUNIONSTORE sums scores by default and stores the result', async () => {
    await withOps(async (c, k) => {
      ...
      const n = await c.zunionstore(k('dest'), '2', k('z1'), k('z2'))
      assert.strictEqual(n, 4)
      assert.deepStrictEqual(
        await c.zrange(k('dest'), '0', '-1', 'WITHSCORES'),
        ['a', '1', 'b', '12', 'c', '23', 'd', '30'],
      )
    })
  })
  ...
  test('ZUNIONSTORE applies WEIGHTS before aggregating', async () => {
    await withOps(async (c, k) => {
      ...
      await c.zunionstore(k('dest'), '2', k('z1'), k('z2'), 'WEIGHTS', '2', '3')
      assert.deepStrictEqual(
        await c.zrange(k('dest'), '0', '-1', 'WITHSCORES'),
        ['a', '2', 'b', '34', 'c', '66', 'd', '90'],
      )
  ...
  test('ZUNIONSTORE honors AGGREGATE MIN and MAX', async () => {
    await withOps(async (c, k) => {
      ...
      await c.zunionstore(k('dest'), '2', k('z1'), k('z2'), 'AGGREGATE', 'MIN')
      assert.deepStrictEqual(
        await c.zrange(k('dest'), '0', '-1', 'WITHSCORES'),
        ['a', '1', 'b', '2', 'c', '3', 'd', '30'],
      )

      await c.zunionstore(k('dest'), '2', k('z1'), k('z2'), 'AGGREGATE', 'MAX')
      assert.deepStrictEqual(
        await c.zrange(k('dest'), '0', '-1', 'WITHSCORES'),
        ['a', '1', 'b', '10', 'c', '20', 'd', '30'],
      )
  ...
  test('ZUNIONSTORE treats a plain set source as scores of 1', async () => {
    await withOps(async (c, k) => {
      ...
      await c.zunionstore(k('dest'), '2', k('z1'), k('s1'))
      assert.deepStrictEqual(
        await c.zrange(k('dest'), '0', '-1', 'WITHSCORES'),
        ['x', '1', 'a', '2', 'b', '3', 'c', '3'],
      )
  ...
  test('ZUNIONSTORE resets a SUM that becomes NaN (inf + -inf) to 0', async () => {
    await withOps(async (c, k) => {
      ...
      await c.zunionstore(k('dest'), '2', k('za'), k('zb'))
  ...
  test('ZINTERSTORE keeps only common members, summing scores', async () => {
    await withOps(async (c, k) => {
      ...
      const n = await c.zinterstore(k('dest'), '2', k('z1'), k('z2'))
  ...
  test('ZUNION returns the union, with and without WITHSCORES', async () => {
    await withOps(async (c, k) => {
      ...
      assert.deepStrictEqual(await c.zunion('2', k('z1'), k('z2')), [
        'a',
        'b',
        'c',
        'd',
      ...
      assert.deepStrictEqual(
        await c.zunion('2', k('z1'), k('z2'), 'WITHSCORES'),
        ['a', '1', 'b', '12', 'c', '23', 'd', '30'],
      )
    })
  })
  ...
  test('ZINTER returns the intersection WITHSCORES', async () => {
    await withOps(async (c, k) => {
      ...
      assert.deepStrictEqual(
        await c.zinter('2', k('z1'), k('z2'), 'WITHSCORES'),
        ['b', '12', 'c', '23'],
      )
    })
  })
  ...
  test('ZDIFF returns members of the first set not in the rest', async () => {
    await withOps(async (c, k) => {
      ...
      assert.deepStrictEqual(await c.zdiff('2', k('z1'), k('z2')), ['a'])
      assert.deepStrictEqual(
        await c.zdiff('2', k('z1'), k('z2'), 'WITHSCORES'),
        ['a', '1'],
      )
    })
  })
  ...
  test('ZDIFFSTORE stores the difference and returns its cardinality', async () => {
    await withOps(async (c, k) => {
      ...
      const n = await c.zdiffstore(k('dest'), '2', k('z1'), k('z2'))
  ...
  test('ZINTERCARD counts the intersection and honors LIMIT', async () => {
    await withOps(async (c, k) => {
      ...
      assert.strictEqual(await c.zintercard('2', k('z1'), k('z2')), 2)
      assert.strictEqual(
        await c.zintercard('2', k('z1'), k('z2'), 'LIMIT', '1'),
        1,
      )
      // LIMIT 0 means no limit
      assert.strictEqual(
        await c.zintercard('2', k('z1'), k('z2'), 'LIMIT', '0'),
        2,
      )
    })
  })
