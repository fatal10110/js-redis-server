...
describe(`LMPOP / BLMPOP Integration (${testRunner.getBackendName()})`, () => {
  ...
  test('LMPOP pops from the first non-empty key with the requested side and count', async () => {
    ...
    assert.deepStrictEqual(
      await client1!.lmpop('3', empty, first, second, 'LEFT', 'COUNT', '2'),
      [first, ['a', 'b']],
    )
    assert.deepStrictEqual(await client1!.lrange(first, 0, -1), ['c'])
    assert.deepStrictEqual(await client1!.lrange(second, 0, -1), ['x', 'y'])

    assert.deepStrictEqual(await client1!.lmpop('2', first, second, 'RIGHT'), [
      first,
      ['c'],
    ])
    ...
    assert.deepStrictEqual(
      await client1!.lmpop('2', first, second, 'RIGHT', 'COUNT', '5'),
      [second, ['y', 'x']],
    )
  ...
  test('LMPOP returns null when all keys are empty or missing', async () => {
    ...
    assert.strictEqual(await client1!.lmpop('2', first, second, 'LEFT'), null)
  })
  ...
  test('BLMPOP returns immediately when a list is non-empty', async () => {
    ...
    assert.deepStrictEqual(
      await client1!.blmpop('1', '2', empty, list, 'RIGHT', 'COUNT', '2'),
      [list, ['three', 'two']],
    )
  ...
  test('BLMPOP blocks then returns when a push arrives', async () => {
    ...
    const blockPromise = client1!.blmpop(
      '5',
      '2',
      first,
      second,
  ...
  test('BLMPOP timeout returns null', async () => {
    ...
    assert.strictEqual(
      await client1!.blmpop('0.1', '2', first, second, 'LEFT'),
      null,
    )
  })
  ...
  test('LMPOP / BLMPOP error paths match Redis', async () => {
    ...
    await assert.rejects(
      () => client1!.lmpop('0', list, 'LEFT'),
      errorWithMessage('ERR numkeys should be greater than 0'),
    )
    await assert.rejects(
      () => client1!.lmpop('abc', list, 'LEFT'),
      errorWithMessage('ERR numkeys should be greater than 0'),
    )
    await assert.rejects(
      () => client1!.lmpop('2', list, other),
      errorWithMessage('ERR syntax error'),
    )
    await assert.rejects(
      () => client1!.lmpop('1', list, 'MIDDLE'),
      errorWithMessage('ERR syntax error'),
    )
    await assert.rejects(
      () => client1!.lmpop('1', list, 'LEFT', 'LIMIT', '1'),
      errorWithMessage('ERR syntax error'),
    )
    await assert.rejects(
      () => client1!.lmpop('1', list, 'LEFT', 'COUNT'),
      errorWithMessage('ERR syntax error'),
    )
    await assert.rejects(
      () => client1!.lmpop('1', list, 'LEFT', 'COUNT', '0'),
      errorWithMessage('ERR count should be greater than 0'),
    )
    await assert.rejects(
      () => client1!.lmpop('1', list, 'LEFT', 'COUNT', '-1'),
      errorWithMessage('ERR count should be greater than 0'),
    )
    await assert.rejects(
      () => client1!.lmpop('1', list, 'LEFT', 'COUNT', 'abc'),
      errorWithMessage('ERR count should be greater than 0'),
    )
    await assert.rejects(
      () => client1!.lmpop('1', stringKey, 'LEFT'),
      errorWithMessage(
    ...
    await assert.rejects(
      () => client1!.blmpop('-1', '1', list, 'LEFT'),
      errorWithMessage('ERR timeout is negative'),
    )
    await assert.rejects(
      () => client1!.blmpop('abc', '1', list, 'LEFT'),
