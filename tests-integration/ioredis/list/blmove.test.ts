...
describe(`BLMOVE Integration (${testRunner.getBackendName()})`, () => {
  ...
  test('BLMOVE returns immediately when source is non-empty', async () => {
    ...
    assert.strictEqual(
      await client1!.blmove(src, dst, 'LEFT', 'RIGHT', '1'),
      'a',
    )
    ...
    assert.strictEqual(
      await client1!.blmove(src, dst, 'RIGHT', 'LEFT', '1'),
      'c',
    )
  ...
  test('BLMOVE timeout returns null', async () => {
    ...
    const result = await client1!.blmove(src, dst, 'LEFT', 'RIGHT', '0.1')
  ...
  test('BLMOVE rotates a list onto itself', async () => {
    ...
    assert.strictEqual(
      await client1!.blmove(key, key, 'RIGHT', 'LEFT', '1'),
      '3',
    )
